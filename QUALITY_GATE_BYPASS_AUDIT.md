...
| §2.1 | `alerting_service/gateway/ack_escalation.py`        | `except Exception:`     | Architectural exception | 2026-05  | Escalation cron tick failure must not kill the loop; logged w/ exc_info                                                                                                                         |
...
| §2.1 | `alerting_service/gateway/incident_persister.py`    | `except Exception:`     | Architectural exception | 2026-05  | GCS write failures must not block state machine transitions; logged via \_logger.exception(); per IncidentPersister persist contract                                                            |
| §3.1 | `alerting_service/notifiers/router.py`              | `MAX_FILE_LINES=1100`   | Architectural exception | 2026-05  | ISS-032: router.py is a large orchestration entrypoint; P0.12-P0.14 (route_incident, SEV0 override, recovery gate) push it to ~1000L; splitting would break cohesion of incident dispatch logic |
