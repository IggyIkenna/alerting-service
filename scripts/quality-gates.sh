...
EL_OLD=$(rg "from unified_cloud_services[. ].*(log_event|setup_events|setup_cloud_logging|observability)" \
...
UCS_DOMAIN=$(rg 'from unified_cloud_services import[^#]*?(InstrumentsDomainClient|ExecutionDomainClient|create_instruments_client|create_execution_client)' \
    --type py --glob "!tests/**" "$SOURCE_DIR" 2>/dev/null || true)
if [ -n "$UCS_DOMAIN" ]; then
    log_fail "Domain clients must come from unified_domain_services, not unified_cloud_services"
    echo "$UCS_DOMAIN" | head -3
    CODEX_VIOLATIONS=$((CODEX_VIOLATIONS + 1))
else
    log_success "Domain clients imported from unified_domain_services"
fi

# Check 24: ID conventions from wrong package
ID_CONV=$(rg 'from unified_cloud_services import[^#]*?(generate_strategy_id|generate_config_id|CATEGORIES|MODES|TIMEFRAMES)' \
    --type py "$SOURCE_DIR" 2>/dev/null || true)
if [ -n "$ID_CONV" ]; then
    log_fail "ID convention functions must come from unified_config_interface, not unified_cloud_services"
    echo "$ID_CONV" | head -3
    CODEX_VIOLATIONS=$((CODEX_VIOLATIONS + 1))
else
...
# Check 25: pip-audit (non-blocking if not installed)
echo -n "Checking dependency vulnerabilities (pip-audit)... "
if command -v pip-audit &>/dev/null; then
    pip-audit 2>/dev/null && echo -e "${GREEN}PASS${NC}" || log_warn "pip-audit found vulnerabilities (review deps)"
elif $PYTHON_CMD -c "import pip_audit" 2>/dev/null; then
    $PYTHON_CMD -m pip_audit 2>/dev/null && echo -e "${GREEN}PASS${NC}" || log_warn "pip-audit found vulnerabilities"
else
    log_warn "pip-audit not installed (add to dev deps: uv pip install pip-audit)"
fi
