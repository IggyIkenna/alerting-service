...
def route_incident_envelope_to_fallbacks(
    envelope: object,  # IncidentEnvelope — type hint as object to avoid hard UAC import at module level
    *,
    ...
    Args:
        envelope: IncidentEnvelope instance (Pydantic, frozen).
        immediate_sev0_overrides: Names of ImmediateSev0Override predicates
            that fired (closed-set 7-tuple). Non-empty = force SEV0 routing.
        provider_in_fallback_mode: Set by ProviderHealthProbe on_fallback_change
            callback when the primary provider (PagerDuty) probe fails.
    ...
    """
    from unified_api_contracts.alerting import AlertSeverity  # local import — keeps module-level light
    ...
    if should_fire_twilio_voice:
        ...
        if twilio_sid and twilio_token and twilio_from and twilio_to:
            ...
            logger.warning(
                "route_incident_envelope_to_fallbacks: Twilio not configured (SM creds missing); skipping voice for incident=%s",
                getattr(envelope, "incident_key", "?"),
            )
    ...
    if should_fire_pager and config.physical_pager_vendor_name:
        ...
        try:
            ...
            if pager_cls is PhysicalPagerNotifierGsmSiren:
                notifier = PhysicalPagerNotifierGsmSiren(
            ...
            pager_result = notifier.send(
                severity=severity if severity is not None else AlertSeverity.CRITICAL,
            ...
            result["physical_pager"] = pager_result.ok
